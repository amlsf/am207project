...
def entropy(cl=4, nc=6, code=None, silent=False):
    '''
    Iteratively selects the best move based on the Shannon entropy of all remaining possible guesses.
    Entropy is a measure commonly used in information theory to quantify the average amount of
    information generated by a distribution.
    In this case, the distribution represents the expected partitioning over possible game responses
    created by a given guess. By maximizing the Shannon (i.e. log2) entropy, we select the guess
    (or one of the guesses) that results in the most even distribution spread across response categories.
